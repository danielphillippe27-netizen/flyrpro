...
async function runDiamondBuild(payload) {
  ...
  const scriptPath = path.join(__dirname, "scripts", "build-diamond-pmtiles.ts");
  ...
  console.log("[DiamondBuild] Starting PMTiles build", {
    campaignId,
  ...
  await new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      if (code === 0) resolve();
      else reject(new Error(`Diamond build exited with code ${code}`));
    });
  });

  console.log("[DiamondBuild] PMTiles build completed", { campaignId });
  return { campaign_id: campaignId, built: true };
}

exports.handler = async (event) => {
  try {
    if (event?.diamondAsync === true) {
      const result = await runDiamondBuild(parseBody(event));
    ...
    console.error("[DiamondBuild] Request failed", error);
    return json(500, {
