...
async function queryAddresses({ state, polygon, limit }) {
  ...
  if (addrSource.isCanadian) {
    ...
    primarySql = `
      ...
      SELECT 
        street_no as house_number,
        COALESCE(NULLIF(TRIM(street), ''), TRIM(str_name || ' ' || COALESCE(str_type, '') || ' ' || COALESCE(str_dir, ''))) as street_name,
        unit,
        city,
        '${stateCode}' as state,
        postal_code,
        ...
        COALESCE(NULLIF(TRIM(street_no || ' ' || COALESCE(street, str_name || ' ' || COALESCE(str_type, '') || ' ' || COALESCE(str_dir, ''))), ' '), street_no || ' ' || str_name) as formatted
      FROM raw
