...
// Canadian province codes (2-letter lowercase)
const CANADIAN_PROVINCES = new Set(['on', 'bc', 'ab', 'qc', 'mb', 'sk', 'ns', 'nb', 'pe', 'nl', 'nt', 'nu', 'yt']);

/**
 * Get the S3 path for addresses based on state/province
 * - US states: Use master_addresses_parquet (Parquet format)
 * - Canadian provinces: Use silver/ca/{province}/addresses.csv (CSV format)
 */
function addressesPathForState(state) {
  const stateLower = state.toLowerCase();
  
  if (CANADIAN_PROVINCES.has(stateLower)) {
    // Canadian province: Use Silver tier CSV data
    return {
      path: `s3://${ADDRESSES_BUCKET}/silver/ca/${stateLower}/addresses.csv`,
      format: 'csv',
      isCanadian: true
    };
  }
  
  // US state: Use master_addresses_parquet (Parquet format)
  return {
    path: `s3://${ADDRESSES_BUCKET}/master_addresses_parquet/state=${state}/data_*.parquet`,
    format: 'parquet',
    isCanadian: false
  };
}
...
async function queryAddresses({ state, polygon, limit }) {
  ...
  if (addrSource.isCanadian) {
    // Canadian Silver: schema varies by province (BC = street_number/street_name, StatCan ODA = street_no/street/str_name/...)
    const stateCode = (state || 'ON').toString().toUpperCase().slice(0, 2);
    const describeSql = `DESCRIBE SELECT * FROM read_csv_auto('${addrSource.path}', all_varchar=true, quote='"', ignore_errors=true) LIMIT 1;`;
    const cols = await queryWithIam(describeSql, REGION);
    const names = new Set((cols || []).map((c) => (c.column_name || c.name || '').toLowerCase()));

    const hasStreetNumber = names.has('street_number');
    const hasStreetNo = names.has('street_no');
    const houseNumberCol = hasStreetNumber ? 'street_number' : hasStreetNo ? 'street_no' : null;
    const hasStreetName = names.has('street_name');
    const hasStreet = names.has('street');
    const hasStrName = names.has('str_name');
    const hasLongitude = names.has('longitude');
    const hasLatitude = names.has('latitude');
    const hasCity = names.has('city');
    const hasUnit = names.has('unit');
    const hasPostalCode = names.has('postal_code');

    if (!houseNumberCol || (!hasStreetName && !hasStreet && !hasStrName) || !hasLongitude || !hasLatitude) {
      throw new Error(
        `Canadian addresses CSV missing required columns. Has: ${Array.from(names).join(', ')}. Need: (street_number or street_no), (street_name or street or str_name), longitude, latitude.`
      );
    }

    // Street name: prefer street_name (BC), else street, else str_name || str_type || str_dir (ODA)
    const streetNameExpr = hasStreetName
      ? 'TRIM(COALESCE(street_name, \'\'))'
      : hasStreet
        ? 'TRIM(COALESCE(street, \'\'))'
        : 'TRIM(COALESCE(str_name, \'\') || \' \' || COALESCE(str_type, \'\') || \' \' || COALESCE(str_dir, \'\'))';
    const cityExpr = hasCity ? 'city' : "''";
    const unitExpr = hasUnit ? 'unit' : "''";
    const postalExpr = hasPostalCode ? 'postal_code' : "''";
    const formattedExpr = hasStreetName
      ? `COALESCE(NULLIF(TRIM(${houseNumberCol} || ' ' || street_name), ' '), street_name)`
      : hasStreet
        ? `COALESCE(NULLIF(TRIM(${houseNumberCol} || ' ' || street), ' '), ${houseNumberCol} || ' ' || COALESCE(street, ''))`
        : `COALESCE(NULLIF(TRIM(${houseNumberCol} || ' ' || COALESCE(str_name, '') || ' ' || COALESCE(str_type, '') || ' ' || COALESCE(str_dir, '')), ' '), ${houseNumberCol} || ' ' || COALESCE(str_name, ''))`;
    ...
    sql = `
      ...
      SELECT 
        ${houseNumberCol} as house_number,
        ${streetNameExpr} as street_name,
        ${unitExpr} as unit,
        ${cityExpr} as city,
        '${stateCode}' as state,
        ${postalExpr} as postal_code,
        CAST(longitude AS DOUBLE) as longitude,
        CAST(latitude AS DOUBLE) as latitude,
        ${formattedExpr} as formatted
      FROM raw
      WHERE (CAST(longitude AS DOUBLE) BETWEEN ${bufferedMinX} AND ${bufferedMaxX}) 
        AND (CAST(latitude AS DOUBLE) BETWEEN ${bufferedMinY} AND ${bufferedMaxY})
        AND ST_Intersects(ST_GeomFromText('${wkt}'), ST_Point(CAST(longitude AS DOUBLE), CAST(latitude AS DOUBLE)))
      LIMIT ${limit}
    `;
  } else {
  ...
  return {
    ...
    features: rows.map(r => ({
      ...
      properties: {
        layer: "addresses",
        id: r.gers_id || `${r.house_number}_${r.street_name}_${r.city}`,
        label: r.formatted || `${r.house_number || ''} ${r.street_name || ''}`.trim(),
        gers_id: r.gers_id,
