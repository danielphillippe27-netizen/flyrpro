...
| `20260617141500_create_demo_events.sql` | 2026-06-17 | Creates append-only analytics events for public demo sessions, intentionally without a `demo_links` FK so generic and fallback slugs can still be tracked. | `demo_events` | No DROP statements. Creates indexes for slug/session readouts and enables RLS without anon/authenticated policies. |
