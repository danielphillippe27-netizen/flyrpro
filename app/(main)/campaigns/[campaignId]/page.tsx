...
import { CampaignActivityView } from '@/components/campaigns/CampaignActivityView';
...
function CampaignAddressStatusList({
  title,
  addresses,
  emptyMessage,
}: {
  ...
  const sorted = [...addresses].sort((a, b) => {
    const aScans = a.scans ?? 0;
    const bScans = b.scans ?? 0;
    if (bScans !== aScans) return bScans - aScans;
    return (a.formatted || a.address || '').localeCompare(b.formatted || b.address || '');
  });
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-left text-muted-foreground">
                ...
                <th className="pb-2 pr-3 font-medium">Scans</th>
                <th className="pb-2 pr-3 font-medium">Visited</th>
              </tr>
            </thead>
            <tbody>
              {sorted.map((addr) => {
                const status = getStatusBadge(addr);
                return (
                  <tr key={addr.id} className="border-b border-border/50">
                    <td className="py-2 pr-3 text-foreground">{addr.formatted || addr.address || '—'}</td>
                    <td className="py-2 pr-3">
                      <Badge className={`text-xs ${status.color}`}>{status.label}</Badge>
                    </td>
                    <td className="py-2 pr-3 text-foreground">{addr.scans ?? 0}</td>
                    <td className="py-2 pr-3 text-foreground">{addr.visited ? 'Yes' : 'No'}</td>
                  </tr>
                );
              })}
            </tbody>
...
export default function CampaignDetailPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [addressesData, statsData, contactsData] = await Promise.all([
  ...
  const appointmentAddresses = dedupedAddresses.filter((addr) => addr.address_status === 'appointment');
  const followUpAddresses = dedupedAddresses.filter((addr) => addr.address_status === 'future_seller');
      ...
      <main className="w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <Tabs defaultValue="map" className="w-full">
          ...
          <TabsContent value="appointments" className="mt-4">
            <CampaignAddressStatusList
              title="Appointments"
              addresses={appointmentAddresses}
              emptyMessage="No appointment addresses yet. Set address status to Appointment in the field app."
            />
          </TabsContent>
          ...
          <TabsContent value="follow-up" className="mt-4">
            <CampaignAddressStatusList
              title="Follow Up"
              addresses={followUpAddresses}
              emptyMessage="No follow-up addresses yet. Set address status to Future Seller in the field app."
            />
          </TabsContent>
          ...
          <TabsContent value="activity" className="mt-4">
            <CampaignActivityView campaignId={campaignId} />
          </TabsContent>
