...
export default function CreateCampaignPage() {
  ...
  const add2DBuildingsLayer = (m: mapboxgl.Map) => {
    ...
    for (const layer of layers) {
      const maybeLayout = (layer as { layout?: Record<string, unknown> }).layout;
      if (layer.type === 'symbol' && maybeLayout?.['text-field']) {
  ...
  const handleSubmit = async (e?: React.FormEvent | React.MouseEvent) => {
    ...
    try {
      ...
      const errorMessage = errorObj?.message || errorObj?.details || errorObj?.hint || 'Unknown error occurred';
      const errorCode = errorObj?.code ? ` (${errorObj.code})` : '';
