...
function isAuthorized(request: NextRequest): boolean {
  ...
  if (secret) {
    return (
      ...
      request.headers.get('x-white-gold-build-secret') === secret ||
...
function commandDescription(command: string | null): string {
  return command ? 'WHITE_GOLD_BUILD_COMMAND' : 'npm run white-gold:build';
}
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => null)) as WhiteGoldBuildRequest | null;
  const campaignId =
  ...
  const customCommand = process.env.WHITE_GOLD_BUILD_COMMAND?.trim() || null;
  ...
  const child = customCommand
    ...
    : spawn('npm', ['run', 'white-gold:build', '--', ...args], {
        cwd: process.cwd(),
        detached: true,
        env: process.env,
        stdio: 'ignore',
  ...
  console.log('[WhiteGoldBuild] Queued background build:', {
    campaignId,
    pid: child.pid,
