...
function parseGoldGeometry(raw: unknown): GeoJSON.Point | null {
  if (!raw) return null;
  ...
  const maybe = parsed as { type?: string; coordinates?: unknown };
  if (maybe.type !== 'Point' || !Array.isArray(maybe.coordinates) || maybe.coordinates.length < 2) {
    return null;
  }

  const lon = Number(maybe.coordinates[0]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: campaign, error: campaignError } = await supabaseAdmin
      .from('campaigns')
      .select('owner_id, region, territory_boundary')
      .eq('id', campaign_id)
      .single();
    ...
    // Use campaign.region when set; otherwise derive from polygon (e.g. Vancouver → BC not ON)
    const polygonForRegion = (body.polygon as GeoJSON.Polygon) ?? (campaign.territory_boundary as GeoJSON.Polygon | null);
    const derivedRegion = polygonForRegion ? regionFromPolygon(polygonForRegion) : null;
    const effectiveRegion = campaign.region?.trim() || derivedRegion || 'ON';
    const regionCode = effectiveRegion.toUpperCase();

    if (!campaign.region?.trim() && derivedRegion) {
      await supabaseAdmin.from('campaigns').update({ region: derivedRegion }).eq('id', campaign_id);
      console.log('[generate-address-list] Set campaign region from polygon:', derivedRegion);
    }
    ...
    if (polygon) {
      ...
      try {
        ...
        if (goldAddresses && goldAddresses.length > 0) {
          ...
          // Some RPC environments return geom_geojson as text, others as already-parsed JSON.
          const goldAddressRows = goldAddresses as GoldAddressRow[];
          const invalidGeometryRows: number[] = [];
          addressFeatures = goldAddressRows
            .map((addr: GoldAddressRow, index: number) => {
              const geometry = parseGoldGeometry(addr.geom_geojson);
              if (!geometry) {
                invalidGeometryRows.push(index);
                return null;
              }
              const streetParts = [addr.street_number, addr.street_name]
                .filter((v) => v !== null && v !== undefined && String(v).trim().length > 0)
                .map((v) => String(v).trim());
              const locality = (addr.city || '').trim();
              const formatted = streetParts.length > 0
                ? `${streetParts.join(' ')}${locality ? `, ${locality}` : ''}`
                : locality || `gold_${addr.id}`;
              return {
                type: 'Feature' as const,
                geometry,
                properties: {
                  gers_id: `gold_${addr.id}`, // Generate pseudo-GERS ID
                  house_number: addr.street_number?.toString() || undefined,
            ...
            .filter((feature): feature is AddressFeature => feature !== null);

          if (invalidGeometryRows.length > 0) {
            console.warn(
              `[generate-address-list] Skipped ${invalidGeometryRows.length} Gold rows with invalid geometry`
            );
          }
        } else {
        ...
        console.warn('[generate-address-list] Gold query failed:', message);
      }
