...
function getBuildingsCount(buildings: unknown): number {
  if (Array.isArray(buildings)) return buildings.length;
  if (
    buildings &&
    typeof buildings === 'object' &&
    Array.isArray((buildings as { features?: unknown[] }).features)
  ) {
    return (buildings as { features: unknown[] }).features.length;
  }
  return 0;
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: campaign, error: campaignError } = await supabase
      .from('campaigns')
      .select('owner_id, workspace_id, territory_boundary, region')
      .eq('id', campaign_id)
      .single();
    ...
    const result = await retryWithBackoff(async () => {
      let addressesToInsert: AddressLike[] = [];
      let goldBuildings: unknown = null;
      ...
      if (snapshotValid) {
        ...
        addressesToInsert = AddressAdapter.normalizeArray(lambdaAddresses, campaign_id!) as AddressLike[];
        goldBuildings = [];
        ...
        const goldBuildingCount = getBuildingsCount(goldBuildings);
        ...
        if (goldResult.snapshot) {
          ...
          console.log(`[Provision] Using ${goldBuildingCount} Gold Standard buildings`);
        }
      }
      ...
      if (normalizedBuildingsGeoJSON.features?.length > 0) {
        ...
        console.log('[Provision] Sample building:', {
          ...
          source: getBuildingsCount(goldBuildings) > 0 ? 'gold' : 'lambda'
        });
      }
      ...
      if (insertedCount >= 2) {
        ...
        try {
          const { data: allAddresses } = await supabase
            .from('campaign_addresses')
            .select('id, geom, house_number, street_name, formatted, gers_id, building_id')
            .eq('campaign_id', campaign_id);
      ...
      try {
        if (getBuildingsCount(goldBuildings) > 0) {
          // Use consolidated linker: polygon from DB, Gold + Silver in one call (worked before)
          console.log('[Provision] Using link_campaign_addresses_all (polygon from DB)...');
          const { data: linkResult, error: linkError } = await supabase
            .rpc('link_campaign_addresses_all', { p_campaign_id: campaign_id });
          
          if (linkError) {
            console.error('[Provision] Linker failed:', linkError.message);
          } else {
            const row = linkResult?.[0];
            const goldExact = Number(row?.gold_exact ?? 0);
            const goldProx = Number(row?.gold_prox ?? 0);
            const silverExact = Number(row?.silver_exact ?? 0);
            const silverProx = Number(row?.silver_prox ?? 0);
            const total = Number(row?.total_linked ?? 0);
            const exact = goldExact + silverExact;
            const proximity = goldProx + silverProx;
            
            console.log(`[Provision] Linker complete: Gold ${goldExact}+${goldProx}, Silver ${silverExact}+${silverProx}, total ${total}`);
            
            spatialJoinSummary = {
              matched: total,
              orphans: insertedCount - total,
              suspect: 0,
              avgConfidence: total > 0 ? (exact * 1.0 + proximity * 0.8) / total : 0,
              coveragePercent: insertedCount > 0 ? (total / insertedCount) * 100 : 0,
              matchBreakdown: {
                containmentVerified: exact,
                containmentSuspect: 0,
                pointOnSurface: 0,
                proximityVerified: proximity,
                proximityFallback: 0,
              },
            };
          }
      ...
      return {
        ...
        buildings_saved: getBuildingsCount(goldBuildings) || snapshot?.counts?.buildings || 0,
