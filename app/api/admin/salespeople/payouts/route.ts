...
import { paySalespersonCommissions } from '@/app/lib/billing/salesperson-payouts';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!result.ok) {
      const status =
        result.code === 'not_found'
          ? 404
          : result.code === 'not_ready' || result.code === 'no_pending_commissions'
            ? 409
            : 500;
    ...
    console.error('[api/admin/salespeople/payouts] POST error:', error);
    return NextResponse.json(
      {
