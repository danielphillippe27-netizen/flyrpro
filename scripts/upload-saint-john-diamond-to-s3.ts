#!/usr/bin/env tsx
/**
 * Build and upload Saint John, NB address and parcel Diamond layers to S3.
 *
 * S3 prefixes:
 *   diamond/addresses/canada/nb/saint-john/
 *   diamond/parcels/canada/nb/saint-john/
 *   diamond/canada/nb/saint-john/diamond-city-manifest.json
 *
 * Usage:
 *   npm run diamond:saint-john
 *   npm run diamond:saint-john -- --dry-run --keep-workdir
 *   npm run diamond:saint-john -- --reuse-existing
 */
...
const outputRoot = path.resolve(
  readFlag('output-root') ?? '../municipal_data/diamond/canada/nb/saint-john'
);

const country = 'canada';
const region = 'nb';
const municipality = 'saint-john';
const bucket =
...
const layers: LayerConfig[] = [
  {
    layer: 'addresses',
    sourceName: 'City of Saint John Civic Addresses',
    sourceDataset: 'Civic_Addresses/FeatureServer/1',
    sourceUrl: 'https://services.arcgis.com/DLk7hgTJeJILuEBg/arcgis/rest/services/Civic_Addresses/FeatureServer/1',
    itemUrl: 'https://www.arcgis.com/home/item.html?id=88d9dbea7fad4767a9870c5666ec4c71',
    minzoom: 10,
    maxzoom: 16,
    pageSize: 4000,
  ...
  {
    layer: 'parcels',
    sourceName: 'City of Saint John Property Parcels',
    sourceDataset: 'Property_Parcels_Public/FeatureServer/0',
    sourceUrl: 'https://services.arcgis.com/DLk7hgTJeJILuEBg/arcgis/rest/services/Property_Parcels_Public/FeatureServer/0',
    itemUrl: 'https://www.arcgis.com/home/item.html?id=ab56024b7c4e4cf38503c91a464b833f',
    minzoom: 10,
    maxzoom: 16,
    pageSize: 500,
    promoteId: 'parcel_id',
...
main().catch((error) => {
  console.error('Saint John Diamond upload failed:', error);
  process.exit(1);
});

async function main() {
  const workdir = path.join(tmpdir(), `flyr-saint-john-diamond-${Date.now()}`);
  const generatedAt = new Date().toISOString();

  console.log('Building Saint John Diamond municipal layers');
  ...
  try {
    ...
    console.log('Saint John Diamond municipal layers are ready.');
    if (dryRun) {
...
function normalizeAddressFeature(feature: ArcGISFeature): Feature | null {
  ...
  const fullAddress = stringValue(attributes.ENCIVSTNAM);
  const streetNumber = stringValue(attributes.Civic_Numb);
  const streetName = stringValue(attributes.Street_Name);
  const streetType = stringValue(attributes.English_Designator);
  ...
  return {
    ...
    properties: cleanProperties({
      address_id: `saint_john_addresses:${objectId}`,
      full_address: fullAddress ?? streetAddress,
      street_address: streetAddress,
      street_number: streetNumber,
      street_name: streetName,
      street_type: streetType,
      civic_type: stringValue(attributes.Civic_Type),
      source: 'saint_john_addresses',
      source_dataset: 'Civic_Addresses/FeatureServer/1',
...
function normalizeParcelFeature(feature: ArcGISFeature): Feature | null {
  ...
  const objectId = stringValue(attributes.OBJECTID);
  const pid = stringValue(attributes.PID);
  const parcelId = pid ? `saint_john_parcels:${pid}` : objectId ? `saint_john_parcels:${objectId}` : null;
  ...
  return {
    ...
    properties: cleanProperties({
      ...
      pid,
      source_id: objectId,
      source: 'saint_john_parcels',
      source_dataset: 'Property_Parcels_Public/FeatureServer/0',
      municipality: 'Saint John',
      region: 'New Brunswick',
      province: 'NB',
      country: 'CA',
      area_sqm: numberValue(attributes.Shape__Area),
      shape_length_m: numberValue(attributes.Shape__Length),
    }),
  };
}
...
function buildTileJSON(
  ...
  return {
    tilejson: '3.0.0',
    name: `FLYR Diamond ${titleCase(config.layer)} - Canada / NB / Saint John`,
    scheme: 'xyz',
    vector_layers: [
      {
        id: config.layer,
    ...
    attribution: 'FLYR / City of Saint John open data',
    metadata: {
