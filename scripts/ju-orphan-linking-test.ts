...
async function applyLinkOnly(
  ...
  await prebuildCampaignMapBundle(supabase, String(decisions[0]?.campaign_id), undefined, {
    forceRebuild: true,
  });
}
...
async function main(): Promise<void> {
  ...
  const rollback = await service.rollbackRun(
    String(firstRun.id),
    String(sourceCampaign.data.owner_id),
    'JU rollback verification before final reapplication'
  );
  if (Number(rollback.rolled_back) !== EXPECTED.exactLinks) {
    throw new Error(`Rollback restored ${String(rollback.rolled_back)} decisions`);
  }
