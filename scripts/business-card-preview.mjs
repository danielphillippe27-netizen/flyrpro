...
createServer(async(req,res)=>{
 ...
 if(/^\/brand\/social\/[a-z]+\.svg$/.test(url.pathname)){res.setHeader('Content-Type','image/svg+xml');res.end(await readFile(join(root,'public',url.pathname)));return;}
 if(url.pathname==='/brand/wolfgrid-logo-white.svg'){res.setHeader('Content-Type','image/svg+xml');res.end(await readFile(join(root,'public/brand/wolfgrid-logo-white.svg')));return;}
