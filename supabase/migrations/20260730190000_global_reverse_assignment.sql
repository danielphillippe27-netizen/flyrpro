-- Apply a parcel-less detached-neighborhood reverse-geocode assignment as one
-- transaction. Source address geometry is intentionally never updated.

CREATE TABLE IF NOT EXISTS public.map_reconciliation_global_batches (
  run_id uuid PRIMARY KEY REFERENCES public.map_reconciliation_runs(id) ON DELETE CASCADE,
...
ALTER TABLE public.map_reconciliation_global_batches ENABLE ROW LEVEL SECURITY;
...
BEGIN
  ...
  -- A protected address or any protected occupant of a target building blocks
  -- the complete transaction. Partial chains/cycles are never applied.
  IF EXISTS (
    SELECT 1
