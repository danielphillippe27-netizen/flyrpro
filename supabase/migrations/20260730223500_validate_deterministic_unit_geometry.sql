CREATE OR REPLACE FUNCTION public.normalize_building_unit_geometry(p_geojson jsonb)
...
DECLARE
  ...
  v_polygon geometry;
BEGIN
  v_input := ST_SetSRID(ST_GeomFromGeoJSON(p_geojson::text), 4326);
  ...
  FROM ST_Dump(
    ST_CollectionExtract(
      CASE
        WHEN ST_IsValid(v_input) THEN v_input
        ELSE ST_MakeValid(v_input)
      END,
      3
    )
  ) AS candidate
  WHERE NOT ST_IsEmpty(candidate.geom)
  ORDER BY ST_Area(candidate.geom) DESC
  LIMIT 1;

  IF v_polygon IS NULL OR NOT ST_IsValid(v_polygon) THEN
    RAISE EXCEPTION 'Unit GeoJSON could not be normalized to a valid polygon';
  END IF;

  RETURN ST_Force2D(v_polygon)::geometry(Polygon, 4326);
END;
$$;
...
BEGIN
  ...
  SELECT
    ...
    'active',
    NULL,
    public.normalize_building_unit_geometry(item->'unit_geometry'),
...
REVOKE ALL ON FUNCTION public.normalize_building_unit_geometry(jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.normalize_building_unit_geometry(jsonb) TO service_role;
...
COMMENT ON FUNCTION public.normalize_building_unit_geometry(jsonb) IS
  'Converts unit GeoJSON to SRID 4326, repairs self-intersections, and returns the largest valid polygon.';
