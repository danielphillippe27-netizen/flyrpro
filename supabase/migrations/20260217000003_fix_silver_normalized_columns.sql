-- ============================================================================
-- FIX: Add missing normalized columns to ref_addresses_silver
-- ============================================================================
...
-- Add postal_code_normalized (uppercase, no spaces)
ALTER TABLE ref_addresses_silver 
ADD COLUMN IF NOT EXISTS postal_code_normalized TEXT 
GENERATED ALWAYS AS (
    upper(regexp_replace(postal_code, '[^A-Z0-9]', '', 'gi'))
) STORED;

-- Create indexes for normalized columns
CREATE INDEX IF NOT EXISTS idx_ref_silver_street_norm 
    ON ref_addresses_silver(street_name_normalized);

CREATE INDEX IF NOT EXISTS idx_ref_silver_lookup_norm 
    ON ref_addresses_silver(street_number_normalized, street_name_normalized, city);
...
WHERE table_name = 'ref_addresses_silver' 
