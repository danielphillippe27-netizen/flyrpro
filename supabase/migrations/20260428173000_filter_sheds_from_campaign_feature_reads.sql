-- Keep shed/outbuilding polygons out of rpc_get_campaign_full_features.
-- The linker filters these now, but this read-time guard prevents stale persisted
-- links or cached feature rows from rendering small non-linkable footprints.
...
'Returns a GeoJSON FeatureCollection for a campaign across gold, campaign-scoped, manual, and free address-point features, excluding hidden/deleted and non-linkable shed/outbuilding polygons.';
