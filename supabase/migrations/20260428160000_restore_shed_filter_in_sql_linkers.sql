-- Restore shed/outbuilding filtering in SQL linkers.
--
-- StableLinkerService historically excluded footprints under 30 m2 before
-- matching. The SQL Gold/Silver linker bypassed that TypeScript filter, so
-- small accessory footprints could win exact, parcel, or proximity matches.
...
COMMENT ON FUNCTION public.is_linkable_building_footprint(geometry, text) IS
'True for main building candidates. Filters sub-30m2 footprints and explicit shed/garage/accessory building types before linker matching.';
...
BEGIN
    ...
    FROM (
        ...
        ORDER BY ca2.id, COALESCE(b.area_sqm, ST_Area(b.geom::geography)) DESC
    ) sub
...
'Fast Gold linker with shed filtering, parcel bridge, and proximity fallback.';
...
BEGIN
    ...
    ORDER BY ca.id, ST_Area(b.geom::geography) DESC
    ON CONFLICT (campaign_id, address_id) DO NOTHING;
...
'Links campaign addresses with shed filtering plus exact, parcel-bridge, and proximity passes across Gold and Silver buildings.';
