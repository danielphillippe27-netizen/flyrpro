...
BEGIN
  ...
  BEGIN
    v_buildings := COALESCE(public.rpc_get_campaign_full_features(p_campaign_id), v_buildings);
  EXCEPTION WHEN OTHERS THEN
    BEGIN
      v_buildings := COALESCE(public.get_campaign_buildings_geojson(p_campaign_id), v_buildings);
    EXCEPTION WHEN OTHERS THEN
