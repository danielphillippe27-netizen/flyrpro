-- Let rpc_get_campaign_full_features return Gold building polygons immediately
-- for map-ready campaigns, before the async linker has populated address
-- building_id values. Shed/outbuilding filtering still applies.
...
BEGIN
    ...
    campaign_building_address_matches AS (
        ...
        WHERE b.campaign_id = p_campaign_id
          ...
          AND (
                LOWER(COALESCE(to_jsonb(b)->>'source', '')) = 'manual'
                OR public.is_linkable_building_footprint(b.geom)
          )
          AND NOT EXISTS (
                SELECT 1
                FROM hidden_public_buildings hpb
    ...
    gold_territory_building_features AS (
        SELECT jsonb_build_object(
            'type', 'Feature',
            'id', b.id::text,
            'geometry', ST_AsGeoJSON(b.geom, 6)::jsonb,
            'properties', jsonb_build_object(
                ...
                'area_sqm', b.area_sqm,
                'building_type', b.building_type,
                'subtype', to_jsonb(b)->>'subtype',
                'feature_type', 'territory_building',
                'feature_status', 'unlinked',
                'match_method', NULL,
                'confidence', NULL,
                'status', 'not_visited',
                'scans_today', 0,
        ...
        JOIN public.ref_buildings_gold b
          ON c.territory_boundary IS NOT NULL
         AND ST_Intersects(b.geom, c.territory_boundary)
    ...
    FROM (
        ...
        UNION ALL
        SELECT feature FROM gold_territory_building_features
        UNION ALL
...
'Returns a GeoJSON FeatureCollection for a campaign across linked gold, unlinked gold territory, campaign-scoped, manual, and free address-point features, excluding hidden/deleted and non-linkable shed/outbuilding polygons.';
