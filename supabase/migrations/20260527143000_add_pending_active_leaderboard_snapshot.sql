...
SET updated_at = COALESCE(updated_at, start_time, NOW())
...
CREATE OR REPLACE FUNCTION public.set_sessions_updated_at_for_leaderboard_pending()
RETURNS TRIGGER
LANGUAGE plpgsql
...
DROP TRIGGER IF EXISTS sessions_set_updated_at_for_leaderboard_pending ON public.sessions;
CREATE TRIGGER sessions_set_updated_at_for_leaderboard_pending
  BEFORE UPDATE ON public.sessions
  FOR EACH ROW
  EXECUTE FUNCTION public.set_sessions_updated_at_for_leaderboard_pending();
...
BEGIN
  ...
  active_sessions AS (
    ...
    UNION ALL
    SELECT
      'workspace:' || s.workspace_id::TEXT AS scope_key,
      s.workspace_id,
      s.user_id,
      s.start_time,
      GREATEST(COALESCE(s.doors_hit, s.completed_count, s.flyers_delivered, 0), 0)::INTEGER AS doorknocks,
  ...
  pending_period AS (
    ...
    WHERE a.scope_key = v_scope_key
      AND public.leaderboard_period_start(p_timeframe, a.start_time) = v_current_period
    GROUP BY a.user_id
...
COMMENT ON FUNCTION public.get_leaderboard(TEXT, TEXT, UUID, INTEGER, INTEGER) IS
  'Canonical leaderboard RPC with finalized rank from leaderboard_rollups and pending active-session snapshot.';
