...
CREATE TABLE IF NOT EXISTS public.territory_iq_area_signals (
  ...
  geography_level text NOT NULL
    CHECK (geography_level IN ('point', 'fsa', 'ward', 'neighbourhood', 'city')),
  area_key text,
  ...
  UNIQUE (source_version_id, source_record_id, signal_key)
);

ALTER TABLE public.campaign_territory_iq_scores
  ADD COLUMN IF NOT EXISTS insights jsonb NOT NULL DEFAULT '[]'::jsonb;

COMMENT ON TABLE public.territory_iq_area_signals IS
  'Versioned municipal Territory IQ signals. Area keys and geometries are estimates and never household facts.';

CREATE INDEX IF NOT EXISTS territory_iq_area_signals_geom_idx
  ON public.territory_iq_area_signals USING gist(geom);
CREATE INDEX IF NOT EXISTS territory_iq_area_signals_area_idx
  ON public.territory_iq_area_signals(geography_level, area_key, signal_key);
CREATE INDEX IF NOT EXISTS territory_iq_area_signals_source_idx
  ON public.territory_iq_area_signals(source_version_id, signal_key);

ALTER TABLE public.territory_iq_area_signals ENABLE ROW LEVEL SECURITY;

CREATE POLICY territory_iq_area_signals_service_manage
  ON public.territory_iq_area_signals FOR ALL
  USING (auth.role() = 'service_role')
  WITH CHECK (auth.role() = 'service_role');

GRANT ALL ON public.territory_iq_area_signals TO service_role;
  ...
  signals AS (
    SELECT jsonb_agg(jsonb_build_object(
      ...
      'geometry', CASE
        ...
        ELSE ST_AsGeoJSON(s.geom::geometry)::jsonb
      END,
      'metrics', s.metrics,
      'source_key', sv.source_key,
      'provider', sv.provider,
      'dataset', sv.dataset_name,
      'version', sv.dataset_version,
    ...
    WHERE c.geom IS NOT NULL
      AND (
        ...
        OR (
          s.geography_level = 'fsa'
          AND s.area_key IN (SELECT fsa FROM campaign_fsas)
        )
      )
  )
