-- Ensure team activity RPC exists with the 7-arg signature used by web:
-- public.get_team_activity_feed(
--   p_workspace_id, p_start_ts, p_end_ts, p_type_filter, p_limit_count, p_offset_count, p_user_id
-- )
--
-- Compatible with both legacy activity_events and newer session_events schemas.
...
BEGIN
  ...
  THEN
    -- Legacy shape: session_events exists but has no workspace_id.
    -- Filter by joining through sessions.session_id -> sessions.workspace_id.
    v_source := 'session_events';
...
COMMENT ON FUNCTION public.get_team_activity_feed(uuid, timestamptz, timestamptz, text, integer, integer, uuid)
  IS 'Team activity feed with optional member filter; compatible with session_events/activity_events.';
