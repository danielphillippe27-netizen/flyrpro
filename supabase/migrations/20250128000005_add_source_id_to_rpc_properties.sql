-- Update rpc_get_buildings_in_bbox to include source_id in building properties
-- This allows the frontend to query contacts by GERS ID when a building is clicked
    ...
    FROM (
        SELECT jsonb_build_object(
            ...
            'properties', jsonb_build_object(
                'id', b.id,
                'source_id', b.source_id, -- GERS ID for linking to contacts
...
-- Update comment
COMMENT ON FUNCTION public.rpc_get_buildings_in_bbox IS 'Returns GeoJSON FeatureCollection of buildings in bounding box. Includes source_id (GERS ID) in properties for linking to contacts. Uses ST_Intersects for proper overlap detection. Handles MultiPolygon geometries correctly. When campaign_id is provided, includes buildings with that campaign_id OR buildings linked to campaign addresses via address_id.';
