...
export async function generateFlyerDesigns(
  ...
  try {
    ...
    // 7. Generate design objects with real images from Gemini
    const designs: FlyerDesign[] = await Promise.all(
      descriptions.slice(0, 3).map(async (description, index) => {
        try {
          const imagePrompt = `Create a professional real estate marketing flyer. ${description}. Style: ${payload.style.replace(/-/g, ' ')}, Tone: ${payload.tone}, Brand color: ${payload.brandColor}, Campaign: ${payload.campaignType.replace(/-/g, ' ')}. Orientation: ${payload.orientation}.`;
          
          const imageDataUrl = await generateGeminiImage({
            prompt: imagePrompt,
            size: '1K',
            aspectRatio: payload.orientation === 'horizontal' ? '4:3' : '3:4',
          });
          ...
          console.error(`Failed to generate image for design ${index}:`, imageError);
          return {
            id: `design-${Date.now()}-${index}`,
