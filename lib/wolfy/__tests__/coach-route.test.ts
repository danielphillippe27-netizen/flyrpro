...
test('field coaching route verifies scopes, reports without paid calls, caches, budget and evidence',async()=>{
 const original=globalThis.fetch;let paid=0,reserves=0,member=true,role='owner',allowed=true,badOutput=false,contextError=false,providerFailure=false,authFailure=false;
 ...
 globalThis.fetch=async(input,init)=>{
  ...
  if(url.includes('/wolfy_coach_cache')){if(init?.method==='POST'){cached=JSON.parse(String(init.body));return response(null,201);}return response(cached);}
  if(url.includes('/responses')){
   paid++;if(providerFailure)throw Error('Provider unavailable');const b=JSON.parse(String(init?.body));
   assert.equal(b.model,'gpt-5-nano');assert.equal(b.store,false);assert.equal(b.max_output_tokens,1600);assert.equal(b.text.format.strict,true);assert.equal(b.tools,undefined);
   const e=JSON.parse(b.input).verifiedEvidence[0];const message=badOutput?'You have 999 leads.':`[[${e.id}]] Review your next step before another session.`;
   return response({id:'resp_test',object:'response',status:'completed',output:[{type:'message',role:'assistant',content:[{type:'output_text',text:JSON.stringify({message}),annotations:[]}]}]});
  }
