...
export class StableLinkerService {
  ...
  async runSpatialJoin(
    ...
    try {
      ...
      const summary = this.generateSummary(matches, addresses.length, orphans.length, {
        conflictCount,
        densityWarningCount,
        executionTimeMs,
      });
  ...
  private generateSummary(
    ...
    const summary: SpatialJoinSummary = {
      ...
      coveragePercent: totalAddresses > 0
        ? Math.round((validMatches.length / totalAddresses) * 100)
        : 0,
      matchBreakdown: {
        containmentVerified,
        containmentSuspect,
