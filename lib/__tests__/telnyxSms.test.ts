...
async function main() {
  ...
  process.env.TELNYX_ALPHANUMERIC_SENDER_ID = 'FLYR';
  ...
  await test('retries with numeric sender when Telnyx rejects alphanumeric sender', async () => {
        ...
        return jsonResponse(
          {
            errors: [
              {
                code: '40306',
                detail: 'Alphanumeric sender ID FLYR is not registered for the destination number +61413911007',
              },
            ],
          },
          422
    ...
    const result = await sendTelnyxSms({
      ...
      to: '+61413911007',
      body: 'Hey Matthew',
      webhookUrl: 'https://www.flyrpro.app/api/telnyx/messaging/status',
    });

    assert.equal(calls.length, 2);
    assert.equal(calls[0].url, 'https://api.telnyx.com/v2/messages');
    assert.equal(calls[0].body.from, 'FLYR');
